...
function ProductsList() {
    ...
    useEffect(() => {
        itemsRef.current = itemsRef.current.slice(0, Object.entries(products).length);
    }, [products]);
        ...
        redeemProduct(productId)
            .then(() => {
                async function handleGetHistory() {
                    ...
                    toast.success(
                        <p>
                            <span style={{ color: "#252F3D" }}>{productName}</span> redeemed successfully
                        </p>
                    );
            ...
            .catch((err) => {
                ...
                toast.error("There was a problem with the transaction", {
                    icon: <img src={process.env.PUBLIC_URL + "assets/icons/system-error.svg"} alt="error-icon" />,
                });
                itemsRef.current[index].style.opacity = "100%";
                setLoading(0);
            });
                                ...
                                >
                                    {loading === item._id ? (
                                        ...
                                        <>
                                            Redeem for
                                            <img
                                                src={process.env.PUBLIC_URL + `/assets/icons/aeropay-3.svg`}
                                                alt="aeropay-3"
                                            />
                                            {item.cost}
                                        </>
                                        ...
                                        <>
                                            You need
                                            <img
                                                src={process.env.PUBLIC_URL + `/assets/icons/aeropay-4.svg`}
                                                alt="aeropay-4"
                                            />
